...
export class LeftSidebarComponent {
  readonly routes: SideRoute[];
  // readonly routePropaganda: SideRoute[];
  readonly empresaService = inject(EmpresaService);

  readonly ano = signal<string>(new Date().getFullYear().toString());

  constructor(private sanitizer: DomSanitizer) {
    //const routePropaganda: SideRoute[] = [
    // {
    //   path: '/app/propaganda-admin',
    //   icon: `<svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5"><path stroke-linecap="round" stroke-linejoin="round" d="M6 20.25h12m-7.5-3v3m3-3v3m-10.125-3h17.25c.621 0 1.125-.504 1.125-1.125V4.875C19.125 4.254 18.621 3.75 18 3.75H5.625c-.621 0-1.125.504-1.125 1.125v11.25c0 .621.504 1.125 1.125 1.125z" /></svg>`,
    //   name: 'Propaganda Admin',
    // },
    //];

    const rawRoutes: SideRoute[] = [
      {
        path: '/app/dashboard',
      ...
      // {
      //   path: '/app/notificacao',
      //   icon: `<svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" class="lucide lucide-bell-icon lucide-bell"><path d="M10.268 21a2 2 0 0 0 3.464 0"/><path d="M3.262 15.326A1 1 0 0 0 4 17h16a1 1 0 0 0 .74-1.673C19.41 13.956 18 12.499 18 8A6 6 0 0 0 6 8c0 4.499-1.411 5.956-2.738 7.326"/></svg>`,
      //   name: 'Notificações',
      // },
      // {
      //   name: 'Configurações',
    ...
    this.routes = rawRoutes.map((route) => ({
      ...route,
      icon: this.sanitizer.bypassSecurityTrustHtml(route.icon as string),
    }));

    // this.routePropaganda = routePropaganda.map((route) => ({
    //   ...route,
    //   icon: this.sanitizer.bypassSecurityTrustHtml(route.icon as string),
    // }));
  }
  ...
  closeSidebar(): void {
    const el = document.getElementById('left-sidebar-drawer') as HTMLInputElement;
    if (el) el.click();
  }
}
