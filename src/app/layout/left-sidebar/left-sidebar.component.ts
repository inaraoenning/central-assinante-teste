...
export class LeftSidebarComponent {
  ...
  constructor(private sanitizer: DomSanitizer) {
    ...
    this.routes = rawRoutes.map((route) => ({
      ...
      submenu: route.submenu?.map((sub) => ({
        ...sub,
        icon: this.sanitizer.bypassSecurityTrustHtml(sub.icon as string),
      })),
    }));
  }
  ...
  isSubmenuOpen(route: SideRoute): boolean {
    if (!route.submenu) return false;
    return route.submenu.some((s) => window.location.pathname.startsWith(s.path));
  }
}
